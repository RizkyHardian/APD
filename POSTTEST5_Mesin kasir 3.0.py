...
def kasir():
    ...
    while user_basket != "keluar":
        if user_basket in inv:
            basket.update({user_basket : inv[user_basket]})
    ...
    print("|| Ini adalah barang-barang yang ada di keranjang anda : ")
    ...
    if answer == "yes":
        ...
        for item in basket:
            total.update({item:basket[item]})
        harga_total=sum(total.values())
    ...
    print("||"+"="*56+"||")
    exit = input("|| Tekan enter untuk kembali!!")
    menu_user()
...
def login():
    ...
    while True:
        if cek_username == username[0] and cek_password == password[0]:
            menu_admin()
        elif cek_username == username[0] and cek_password == password[1]:
            menu_user()
        else:
